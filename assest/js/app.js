...
export const updateWeather = (lat, lon) => {
    ...
    fetchData(url.currentWeather(lat, lon), (currentWeather) => {
        ...
        fetchData(url.forecast(lat, lon), (forecast) => {
            ...
            forecastSection.innerHTML = `<h2 class="title-2" id="forecast-label">5 Days Forecast</h2><div class="card card-lg forecast-card"><ul data-forecast-list></ul></div>`;
            for (let i = 7, len = forecastList.length; i < len; i += 8) {
                ...
                li.innerHTML = `<div class="icon-wrapper"><img src="./assest/images/weather_icons/${icon}.png" width="36" height="36" alt="${description}" class="weather-icon" title="${description}"><span class="span"><p class="title-2">${module.formatTemp(temp_max, tempUnit)}</p></span></div><p class="label-1">${date.getDate()} ${module.monthNames[date.getUTCDay()]}</p><p class="label-1">${module.weekDayNames[date.getUTCDay()]}</p>`;
                forecastSection.querySelector("[data-forecast-list]").appendChild(li);
            }
            loading.style.display = "none";
            container.style.display = "grid"; // Show container after loading
...
settingInputs.forEach(input => {
    input.addEventListener('change', (e) => {
        ...
        let value = e.target.type === 'radio' ? document.querySelector(`input[name="${e.target.name}"]:checked`).value : e.target.value;
