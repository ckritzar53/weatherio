...
export const updateWeather = (lat, lon) => {
    ...
    fetchData(url.currentWeather(lat, lon), (currentWeather) => {
        ...
        fetchData(url.airPollution(lat, lon), (airPollution) => {
            ...
            card.innerHTML = `
                <h2 class="title-2" id="highlights-lable">Today's Highlights</h2>
                <div class="highlight-list">
                            ...
                            <div class="card-list">
                                <div class="card-item">
                                    ...
                                    <div>
                                        ...
                                        <p class="title-1">${module.getTime(sunriseUnixUTC, timezone, timeFormat)}</p>
                                    </div>
                                </div>
                                <div class="card-item">
                                    ...
                                    <div>
                                        ...
                                        <p class="title-1">${module.getTime(sunsetUnixUTC, timezone, timeFormat)}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card card-sm highlight-card">
                        ...
                        <div class="wrapper">
                            ...
                            <p class="title-1">${module.formatPressure(pressure, pressureUnit)}</p>
                        </div>
                    </div>
                    <div class="card card-sm highlight-card">
                        ...
                        <div class="wrapper">
                            ...
                            <p class="title-1">${module.formatDistance(visibility, distanceUnit)}</p>
                        </div>
                    </div>
                    <div class="card card-sm highlight-card">
                        ...
                        <div class="wrapper">
                            ...
                            <p class="title-1">${module.formatTemp(feels_like, tempUnit)}</p>
                        </div>
                    </div>
                </div>
            `;
        ...
        fetchData(url.forecast(lat, lon), (forecast) => {
            ...
            for (const [index, data] of forecastList.entries()) {
                ...
                windLi.innerHTML = `
                    <div class="card card-sm slider-card">
                        <p class="body-3">${module.getTime(dateTimeUnix, timezone, timeFormat)}</p>
                        <img src="./assest/images/weather_icons/direction.png" width="48" height="48" loading="lazy" alt="wind direction" class="weather-icon" style="transform: rotate(${windDirection - 180}deg)">
                        <p class="body-3">${module.formatWind(windSpeed, windUnit)}</p>
                    </div>
                `;
            ...
            for (let i = 7, len = forecastList.length; i < len; i += 8) {
                ...
                li.innerHTML = `
                    ...
                    <p class="label-1">${date.getDate()} ${module.monthNames[date.getUTCDay()]}</p>
            ...
            container.style.display = "grid"; // Show content after loading is complete
...
// --- CORRECTED AND FINALIZED SETTINGS LOGIC ---

// Handle all <select> dropdowns for units
...
// Handle the time format radio buttons specifically
const timeFormatRadios = document.querySelectorAll('input[name="timeFormat"]');
timeFormatRadios.forEach(radio => {
    radio.addEventListener('change', (e) => {
        // When a radio button changes, if it's now checked, save its value
        if (e.target.checked) {
...
// Function to load all preferences when the application starts
const loadPreferences = () => {
    // Load dropdown preferences
    settingSelects.forEach(select => {
