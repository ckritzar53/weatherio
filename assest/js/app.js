...
const loadPreferences = () => {
    settingInputs.forEach(input => {
        ...
        if (savedValue) {
            if (input.type === 'radio') {
                document.querySelector(`input[name="${input.name}"][value="${savedValue}"]`).checked = true;
            } else {
...
settingInputs.forEach(input => {
    input.addEventListener('change', (e) => {
        ...
        if (e.target.type === 'radio') {
            value = document.querySelector(`input[name="${e.target.name}"]:checked`).value;
        } else {
        ...
        // Reload the current weather view to apply changes instantly
        window.location.reload();
    });
});

// Load preferences when the DOM is fully loaded
